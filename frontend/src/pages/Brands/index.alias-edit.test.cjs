...
assert.match(source, /写法别名/)
assert.doesNotMatch(source, /品牌别名/)
assert.doesNotMatch(source, /brand_alias_name/)
...
assert.doesNotMatch(source, /alias_name: trimmedAliasName/)
assert.doesNotMatch(source, /aliasRefreshKey/)
assert.doesNotMatch(source, /expandedRowRender/)
assert.match(source, /<AliasPanel brandCode=\{editingBrand\.brand_code\} onAliasChange=\{refresh\} embedded \/>/)
assert.match(source, /refreshDeps: \[brandCode, refreshKey\]/)
