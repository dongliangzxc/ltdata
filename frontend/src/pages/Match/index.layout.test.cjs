...
assert.doesNotMatch(source, /const TRANSFER_PLATFORM_OPTIONS/, 'transfer modal platform options should come from real task platform values');
assert.doesNotMatch(source, /platformOptions: TRANSFER_PLATFORM_OPTIONS/, 'transfer modal should not inject duplicate fixed platform options');
